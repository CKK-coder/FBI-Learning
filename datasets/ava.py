...
class AVADataLoader(data.DataLoader):
    ...
    def _collate_fn(self, batch):
        ...
        img_path = [_['img_path'] for _ in batch]
...
class AVA(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(frame_indices)):
            ...
            clip.append(img)
        mid_frame_path = os.path.join(path, frame_format%frame_indices[len(frame_indices)//2])
        ...
        return {'clip': clip, 'aug_info': aug_info, 'label': target, 
                'video_name': video_name, 'mid_time': mid_time, 'img_path': mid_frame_path}
...
def GetSingleData(root_path, data, video_name, time, spatial_transform=None, temporal_transform=None):
    ...
    new_clips = []
    for clip in clips:
        new_clip, pad_ratios = batch_pad(clip)
        new_clips.append(new_clip.unsqueeze(0))
    for aug in aug_info:
        aug['pad_ratio'] = [1.0, 1.0]
    target = labels
    return {'clips': new_clips, 'aug_info': [aug_info], 'labels': [target], 
            'filenames': [video_name], 'mid_times': [time], 'img_path': [mid_frame_path]}